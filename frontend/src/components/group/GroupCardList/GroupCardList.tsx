...
const GroupCardList: React.SFC<Props> = ({
  ...
  return (
    <React.Fragment>
      {ownDisplayName === commonDisplayName &&
      ownUsername === commonUserName ? (
        <div className={cx('section-wrapper')}>
          <ul className={cx('section-tab')}>
            <li
              className={cx('item', { active: active === false })}
              onClick={() => onSelectTab(false)}
            >
              공개
            </li>
            <li
              className={cx('item', { active: active === true })}
      ...
      <div className={cx('group-card-list')}>{groupList}</div>
    </React.Fragment>
  );
};
