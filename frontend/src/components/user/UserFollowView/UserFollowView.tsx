...
const ViewTitleItem: React.SFC<{
  ...
  <span className={cx('title', { active })} onClick={() => onSelectTab(value)}>
    {title}
  </span>
);

const UserFollowView: React.SFC<Props> = ({
  followDatas,
  onSelectTab,
  tab,
  onUserMore,
    ...
    <div className={cx('user-follow-view')}>
      <section>
        ...
        {followDatas && (
          <ul>
            <li>
              <span onClick={() => onUserMore(cursor)}>전체보기</span>
            </li>
            <li className={cx('placer')} />
            {followDatas.map((f, i) => {
              const { username, thumbnail, displayName } = f;
              return (
                <li key={i}>
