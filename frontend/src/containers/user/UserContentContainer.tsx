...
class UserContentContainer extends React.Component<Props> {
  ...
  public onUserMore = (value: boolean) => {
    const { ListActions } = this.props;
    value
      ? ListActions.followUsersMore(false)
      : ListActions.followUsersMore(true);
  };
    ...
    return (
      ...
      >
        <Switch>
          ...
          <Route exact path="/@:displayName/pins" component={UserPinList} />
          <Route
            exact
            path="/@:displayName/lockers"
            component={LockerPinList}
          />
          <Route exact path="/@:displayName/groups" component={GroupList} />
        </Switch>
      </UserContent>
    );
  }
