...
export const authCreators = {
  ...
  providerLoginType: createAction(
    PROVIDER_LOGIN_TYPE,
    (provider: string) => provider
  ),
  providerLogin: createPromiseThunk(PROVIDER_LOGIN, (provider: string) =>
    social[provider]()
  ),
  socialLogin: createPromiseThunk(SOCIAL_LOGIN, AuthAPI.socialLoginAPI),
  socialRegister: createPromiseThunk(
    SOCIAL_REGISTER,
    AuthAPI.socialRegisterAPI
  ),
...
type ChangeRegisterFormAction = ReturnType<
  typeof authCreators.changeRegisterForm
>;
type ProviderLoginTypeAction = ReturnType<
  typeof authCreators.providerLoginType
>;
type SendAuthEmailAction = GenericResponseAction<{ isUser: boolean }, string>;
type GetCodeAction = GenericResponseAction<
  { email: string; registerToken: string },
  string
>;
type LocalRegisterAction = GenericResponseAction<
  { user: UserSubState; token: string },
  string
>;
...
type SocialLoginAction = GenericResponseAction<
  { user: UserSubState; token: string },
  string
>;
type SocialRegisterAction = GenericResponseAction<
  { user: UserSubState; token: string },
  string
>;
type VerifySocialAction = GenericResponseAction<
  {
