...
export const pinCreators = {
  ...
  changeInputComment: createAction(
    CHANGE_INPUT_COMMENT,
    (payload: ChangeInputCommentPayload) => payload
  ),
...
type GetPinAction = GenericResponseAction<
  ...
  string
>;
type ChangeInputCommentAction = ReturnType<
  typeof pinCreators.changeInputComment
>;
...
type ListCommentAction = GenericResponseAction<
  ...
  string
>;
type LikePinAction = GenericResponseAction<
  { likes: number; liked: boolean },
  string
>;
type UnLikePinAction = GenericResponseAction<
  { likes: number; liked: boolean },
  string
>;
type GetLikePinAction = GenericResponseAction<
  { likes: number; liked: boolean },
  string
>;
...
export default handleActions<PinState, any>(
  {
    ...
    [REMOVE_TAG]: (state, action: RemoveTagAction) => {
      return produce(state, draft => {
        if (action.payload === undefined) return;
        draft.comment.tags = draft.comment.tags.filter(
          t => t !== action.payload
        );
      });
    },
