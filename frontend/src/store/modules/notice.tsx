...
const GET_NOTICE_MESSAGE_ERROR = 'notice/GET_NOTICE_MESSAGE_ERROR';

export const noticeCreators = {
    noticeCancel: createAction(NOTICE_CANCEL),
    noticeConfirm: createAction(NOTICE_CONFIRM),
    getNoticeMessage: createPromiseThunk(GET_NOTICE_MESSAGE, commonAPI.getNoticeMessageAPI),
}

type GetNoticeMessageAction = GenericResponseAction<{ message: MessageSubState[] }, string>;
...
export default handleActions<NoticeState, any>({
    ...
    [GET_NOTICE_MESSAGE_ERROR]: (state) => {
        return produce(state, (draft) => {
            draft.messages = []
        });
    }
