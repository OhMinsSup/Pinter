...
export const getFollowingAPI = (displayName: string, cursor: boolean | null): Promise<any> => {
    if (cursor) {
        return axios.get(`/follow/${displayName}/following/?${queryString.stringify({ cursor })}`);
    }
    return axios.get(`/follow/${displayName}/following`);
}

export const getFollowerAPI = (displayName: string, cursor: boolean | null): Promise<any> => {
    if (cursor) {
        return axios.get(`/follow/${displayName}/follower/?${queryString.stringify({ cursor })}`);
    }
