...
const serializeComment = (data: any) => {
  const { _id: commentId, user, text, createdAt } = data;
...
const serializeNoticeRoom = (data: any) => {
  const { _id: noticeId, creator, createdAt } = data;
...
const serializeNoticeMessage = (data: any) => {
  const { sender, recipient, notice: noticeId, message, createdAt } = data;
...
const serializeGroups = (data: any) => {
  const { _id: groupId, title, user, activation } = data;
